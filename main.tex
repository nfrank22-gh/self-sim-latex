...
Nothing so far fixes the ratios $\tilde{U}_{k+1}, \tilde{L}_{k+1}$ that the
scaling step \eqref{eq:fw-scale-step} depends on. They are fixed by requiring
the rescaled field to carry prescribed values of two functionals $F_1, F_2$ of
the field, each homogeneous under \eqref{eq:fw-scale-step}: there exist
exponents $p_m, q_m$ with
\begin{equation}\label{eq:fw-homogeneity}
  F_m\!\left[ \frac{1}{\tilde{U}}\, u\!\left( \cdot\, \tilde{L} \right) \right]
...
taking logarithms gives a system linear in the log-ratios,
...
\end{equation}
uniquely solvable precisely when
\begin{equation}\label{eq:fw-constraint-solvable}
  p_1 q_2 - p_2 q_1 \neq 0,
\end{equation}
that is, when the two functionals respond independently to amplitude and to
length rescaling. The functionals themselves are equation-specific
